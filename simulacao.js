...
function toggleWalkMode() {
  ...
  if (walkMode) {
    ...
    if (isMobileTouch) {
      document.body.classList.add('walk-active');
      controls.isLocked = true; // fake lock for mobile to keep animate running
      document.getElementById('zone_joystick_left').style.display = 'block';
      document.getElementById('zone_joystick_right').style.display = 'block';
      document.getElementById('movement-tips').style.display = 'none';
      if (!joystickL) initJoysticks();
    } else {
    ...
    if (isMobileTouch) {
      ...
      document.getElementById('zone_joystick_right').style.display = 'none';
    } else {
...
function initJoysticks() {
  joystickL = nipplejs.create({ zone: document.getElementById('zone_joystick_left'), mode: 'static', position:{left:'25%',bottom:'25%'}, color: '#00f2ff' });
  joystickR = nipplejs.create({ zone: document.getElementById('zone_joystick_right'), mode: 'static', position:{right:'25%',bottom:'25%'}, color: '#7000ff' });
  
  joystickL.on('move', (e, d) => { moveJoy.x = Math.cos(d.angle.radian)*d.distance; moveJoy.y = Math.sin(d.angle.radian)*d.distance; });
  joystickL.on('end', () => { moveJoy.x = 0; moveJoy.y = 0; });
  
  joystickR.on('move', (e, d) => { lookJoy.x = Math.cos(d.angle.radian)*d.distance; lookJoy.y = Math.sin(d.angle.radian)*d.distance; });
  joystickR.on('end', () => { lookJoy.x = 0; lookJoy.y = 0; });
}
...
function animate() {
  ...
  if (walkMode && controls.isLocked) {
    ...
    if (isMobileTouch) {
      if (lookJoy.x !== 0 || lookJoy.y !== 0) {
        const euler = new THREE.Euler(0, 0, 0, 'YXZ');
        euler.setFromQuaternion(camera.quaternion);
        euler.y -= lookJoy.x * 0.0005;
        euler.x += lookJoy.y * 0.0005;
        euler.x = Math.max(-Math.PI/2, Math.min(Math.PI/2, euler.x));
        camera.quaternion.setFromEuler(euler);
      }
      direction.z = -moveJoy.y * 0.02;
      direction.x = -moveJoy.x * 0.02;
      
      if (Math.abs(moveJoy.y) > 0 || Math.abs(moveJoy.x) > 0) {
